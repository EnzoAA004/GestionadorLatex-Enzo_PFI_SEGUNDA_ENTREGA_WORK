...
Antes de recorrer las etapas conviene advertir que la nomenclatura de trabajo no fue uniforme durante todo el desarrollo, y que el documento utiliza tres sistemas de identificación que no deben confundirse entre sí. Esa distinción se detalla en la sección 12.2 y se resume aquí porque condiciona la lectura de toda la sección: las épicas E1 a E10 son unidades académicas derivadas del user research; los identificadores AI-, BE-, DEV-, FE-RD- y FE-P corresponden a backlogs internos de cada repositorio; y los checkpoints P8, P9, P10 y la secuencia P10.5 a P10.9 son bloques coordinados de producto.
...
\hline
P10.7 · Hallazgos por nivel & — & Ocho tipos de hallazgo degenerativo, sujetos a revisión profesional y con estado de despliegue asociado. Las probabilidades internas no se exponen. & Salida asistiva por nivel. Los ocho tipos no tienen el mismo soporte empírico y el flujo real permanece bloqueado por paridad de preprocesamiento. & Implementado, no productivo \\
\hline
...
\hline
P10.9 · Consolidación & — & Congelamiento del backend y del módulo de IA para el traspaso al frontend, con recorrido técnico verificado mediante un runner automatizado. & Frontera contractual común posteriormente integrada en Frontend. & Integrado en Frontend; E2E funcional final 11/11 PASS \\
\hline
\end{longtable}
...
El resultado de P10.5 fue una capacidad de navegación volumétrica que permite recorrer el estudio conservando el contexto de cada salida automática. P10.5-A quedó consolidado como contrato común entre los tres repositorios, mientras que los bloques P10.5-B a P10.5-F fueron implementados y verificados dentro de sus respectivos alcances técnicos. Esto no implica que todas esas capacidades hayan quedado consolidadas simultáneamente sobre una única línea integrada de los tres repositorios. En particular, P10.5-F verificó la persistencia, reapertura y vinculación de resultados dentro del alcance de ese bloque; esta verificación no equivale al Full Product UI E2E pendiente luego de P10.9. La integración del contrato P10.9 en el Frontend fue completada posteriormente. Esta integración no equivale por sí sola al cierre del Full Product UI E2E: esa evidencia se consolidó posteriormente con el recorrido browser E2E final 11/11 PASS, dentro del alcance funcional demo/producto.

\subsection{Estado resultante para la entrega del 50 \%}
\label{anexo:f-estado-entrega-50}

El estado del proyecto al cierre de esta versión admite tres categorías que no deben mezclarse. Este estado corresponde específicamente al hito de entrega del 50\,\% y se conserva aquí como registro histórico; el estado final de la validación profesional y del despliegue Cloud, alcanzado en etapas posteriores del proyecto, se documenta en el Capítulo 13 y en el Capítulo 10 respectivamente.

\textbf{Hecho.} La separación en tres repositorios con responsabilidades delimitadas por contrato; los modelos sagital y axial entrenados, congelados y ejecutables sobre entrada real; el runtime de inferencia con gestión de artefactos verificados por hash; la persistencia de estudios, corridas, assets y revisiones sobre PostgreSQL con migraciones versionadas; la reapertura de estudios ya procesados; la navegación por cortes con el resultado vinculado a su corte de origen; la integración P10.9 en Frontend; la implementación Backend/API/persistencia del dominio Patient; la asociación explícita y auditable de Studies mediante patientId; la persistencia de un flujo visible que todavía usa principalmente subjectRef para historial y longitudinal; y la revisión profesional obligatoria.

\textbf{Pendiente.} La validación profesional sobre el producto integrado, el análisis de negocio, el cierre final de conclusiones y el despliegue Cloud completo. El recorrido funcional Full Product UI E2E ya cuenta con evidencia final.
...
\begin{itemize}
  ...
  \item Ingesta de estudio DICOM completo y procesamiento full-series (Backend + AI Module), línea de hallazgos degenerativos por nivel (P10.7) bloqueada para inferencia automática real, visor navegable e integración P10.9 en Frontend, seguida por el dominio Patient parcial y la asociación Study--Patient validada en Backend; el cierre funcional Full Product UI E2E fue completado.
\end{itemize}

En una etapa posterior se incorporó Patient como entidad de-identificada explícita en Backend, con id interno UUID y patientReference como referencia seudónima de trabajo. Los Studies pueden asociarse mediante patientId y esa relación se persiste. Sin embargo, el Frontend final conserva historial y longitudinal principalmente mediante subjectRef; por eso el dominio Patient queda como implementación parcial y no como gestión browser-level completa.
...
\section{Cierre tecnico final y fuente canonica}
\label{anexo:f-cierre-tecnico-final}

El cierre técnico posterior a la entrega del 50 \% siguió una secuencia trazable: post-E50, extensiones de producto, integración \texttt{integration/final-2026}, reconciliación del Backend y congelamiento en \texttt{main}. La fuente técnica canónica final quedó documentada en el repositorio Backend como \texttt{docs/final/FINAL\_PRODUCT\_TECHNICAL\_SOURCE\_OF\_TRUTH.md}.

Los SHAs finales usados como base documental son: Backend \texttt{33081f7cc3702afe4db04a2b63de4762408b5d50}, Frontend \texttt{ff191e1195a4e6bf06312ae8e266d37894c5b7b1} y AI Module \texttt{22b12e04ce37f5c9ba6a02f356ac2022390bf06e}. En el caso del Backend, ese SHA corresponde a un commit documental sobre el merge funcional previo: distingue la fuente de verdad escrita del estado funcional ya reconciliado.

El estado final no se deriva de un único repositorio. Backend aporta seguridad, persistencia, Patient API, reportes, PDF/JSON, Cloud Run DEV y orquestación; Frontend aporta el flujo visible, historial basado en \texttt{subjectRef}, review panels, longitudinal, Grad-CAM wiring y Playwright final 11/11; AI aporta modelos, runtime, Demo Mode, product-analysis, 67B2/67C/P10.7 con sus gates y limitaciones. Esta separación evita degradar o inflar una capacidad por mirar solo una capa.
