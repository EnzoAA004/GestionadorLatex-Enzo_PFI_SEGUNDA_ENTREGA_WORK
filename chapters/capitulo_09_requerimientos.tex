...
HU-03 & Como profesional, quiero crear o seleccionar un Patient de-identificado y asociar el Study correspondiente para organizar longitudinalmente los casos sin almacenar identificadores clínicos directos innecesarios. & Alta & Implementado \\
...
HU-07 & Como profesional, quiero recorrer todos los cortes del volumen para analizar el contexto alrededor del corte inferido. & Alta & Implementado; P10.9 integrado en Frontend. El E2E local/containerizado del producto está validado (sección~\ref{sec:resultados-validacion-final}); el recorrido completo en Cloud desde el navegador es parcial (sección~\ref{sec:resultados-cloud-e2e}). \\
...
HU-10 & Como profesional, quiero reabrir un estudio ya procesado sin repetir la inferencia para conservar continuidad y evitar cómputo innecesario. & Alta & Implementado; reapertura integrada en Frontend. El E2E local/containerizado del producto está validado (sección~\ref{sec:resultados-validacion-final}); el recorrido completo en Cloud desde el navegador es parcial (sección~\ref{sec:resultados-cloud-e2e}). \\
...
HU-14 & Como profesional, quiero comparar mediciones de estudios previos del mismo paciente de-identificado para observar cambios descriptivos en el tiempo. & Baja & Futuro \\
\hline
\end{longtable}
}
...
RF-08 & Documentar y habilitar progresivamente la comparación con estudios previos asociados al mismo Patient de-identificado. & La evolución longitudinal se mantiene separada del MVP inmediato y define datos, comparabilidad, deltas descriptivos y revisión profesional. & Prioridad: Baja Estado: Futuro Fuente: N-10; HU-14 \\
RF-09 & Permitir la navegación ordenada de todos los cortes del estudio o volumen procesado. & El usuario puede recorrer series y cortes del estudio mediante scroll, teclado y modo cine, con controles de imagen (window/level, zoom) e indicador de corte actual/total; los cortes sin inferencia se muestran como imagen base. & Prioridad: Alta Estado: Implementado (Backend + AI en full-series; P10.9 integrado y visor navegable en Frontend). El E2E local/containerizado del producto está validado (sección~\ref{sec:resultados-validacion-final}); el recorrido completo en Cloud desde el navegador es parcial (sección~\ref{sec:resultados-cloud-e2e}). Fuente: N-12 \\
...
RF-14 & Asociar explícitamente cada Study, identificado mediante caseId o studyId según corresponda, con un Patient de-identificado. & La creación, consulta y eventual reasignación conservan identificadores estables y una relación explícita \texttt{Study.patientId} \ensuremath{\rightarrow} \texttt{Patient.id}, sin exigir nombre, DNI u otro identificador clínico directo. La asociación se audita y no se infiere automáticamente desde \texttt{subjectRef}. & Prioridad: Alta Estado: Implementado Fuente: N-10; HU-03 \\
...
RNF-07 & Los datos personales del paciente deben protegerse conforme a la Ley N.º 25.326. & La identidad longitudinal utilizada por el producto es un Patient de-identificado. Los Studies se asocian mediante \texttt{Study.patientId} \ensuremath{\rightarrow} \texttt{Patient.id}; \texttt{patientReference} funciona como referencia seudónima de trabajo y \texttt{subjectRef} permanece únicamente como atributo opcional de compatibilidad. & Prioridad: Alta Estado: Implementado como criterio de diseño \\
...
HU-03 & N-10, privacidad & RF-14; RNF-07 & Frontend + Backend + PostgreSQL & Patient de-identificado creado o seleccionado y Study asociado explícitamente, sin identificadores clínicos directos obligatorios \\
...
HU-07 & N-12 & RF-09, RF-18, RF-21; RNF-09, RNF-11 & Tres repositorios & Catálogo de cortes full-series y visor navegable integrado en Frontend mediante P10.9; cierre E2E global pendiente. \\
...
HU-14 & N-10 & RF-08, RF-14; RNF-07, RNF-09 & Backend + datos + futura IA & Especificación longitudinal sobre Studies asociados al mismo Patient y deltas descriptivos revisables \\
\hline
\end{longtable}
}
...
\section{Actualizacion tecnica de alcance final}

El cierre tecnico agrega capacidades que no modifican el principio de no diagnostico, pero si amplian la trazabilidad del prototipo: exportacion PDF/JSON, historial por \texttt{subjectRef}, comparacion longitudinal descriptiva en Demo Mode, concordance descriptiva, review analytics, feedback curation, Grad-CAM experimental, chat contextual con proveedor mock y despliegue Cloud DEV con AI privado y persistencia gestionada. Estas capacidades se interpretan con estados diferenciados: validadas tecnicamente, parciales, experimentales, demo-only, bloqueadas o planificadas. En particular, Patient queda parcial por la diferencia entre Backend/API y Frontend visible; Cloud queda validado tecnicamente de punta a punta en DEV (autenticacion privada, persistencia y reproceso sin re-subida), aunque el recorrido browser completo en Cloud permanece parcial por una limitacion de tooling de automatizacion; y las capacidades 67B2, 67C y P10.7 quedan sujetas a sus gates de paridad y validacion.
