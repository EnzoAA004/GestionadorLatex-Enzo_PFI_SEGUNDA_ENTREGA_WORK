...
La Figura \ref{fig:arch-despliegue-google-cloud} sintetiza la arquitectura utilizada durante la validación del entorno DEV en Google Cloud. El Frontend se publica mediante Cloud Run y consume exclusivamente al Backend Green por HTTPS. El Backend actúa como frontera de producto: centraliza autenticación, persistencia y serving de assets, y realiza la invocación del AI Module privado mediante un Google ID Token. La información transaccional se conserva en Cloud SQL, mientras que los inputs, outputs y checkpoints se almacenan en Google Cloud Storage. Secret Manager mantiene la configuración sensible fuera del código y Artifact Registry provee las imágenes de contenedor utilizadas por los servicios de Cloud Run.
