...
\hline
\endfoot
Frontend & React, TypeScript y Vite & Autenticación visual, worklist, alta y consulta de estudios, workspace sagital--axial, visor de cortes, revisión, historial, longitudinal y diagnósticos técnicos. \\
Backend & Spring Boot y Java 21 & Frontera de producto, seguridad, persistencia y orquestación: autenticación, reglas de negocio, auditoría, proxy hacia IA, contratos canónicos, serving de assets, reportes, chat y reapertura. \\
AI Module & Python, FastAPI y PyTorch & Servicio especializado de IA: registry de modelos, manifests, hashing, preprocesamiento, inferencia, catálogo volumétrico, overlays, Grad-CAM experimental y generación de previews. \\
...
Persistencia & PostgreSQL & Usuarios, roles, pacientes, estudios, corridas, revisiones, mediciones, evidencia, chat, feedback, artifacts y eventos de auditoría. \\
...
La persistencia se apoya en un modelo relacional en PostgreSQL organizado alrededor de tres ejes: la identidad y el control de acceso (usuarios y roles), la evidencia de cada análisis (corridas, mediciones, revisiones, assets y catálogo de cortes), y el dominio Patient de-identificado del Backend. Este último existe como entidad persistida y API, pero su adopción por la interfaz final es parcial: el historial y la comparación longitudinal que consume el Frontend siguen operando principalmente con \texttt{subjectRef}. La Figura~\ref{fig:arch-modelo-datos} presenta el diagrama entidad-relación con sus claves primarias, foráneas y cardinalidades.

El modelo separa la identidad técnica de cada estudio de la identidad longitudinal de-identificada cuando esta se encuentra disponible. En Backend, \texttt{Patient} posee un identificador interno UUID y una \texttt{patientReference} seudónima; \texttt{Study} conserva su identificador, \texttt{caseId}, \texttt{subjectRef} y una asociación opcional mediante \texttt{patientId}. La asociación \texttt{Patient--Study} está implementada, persistida y validada a nivel API. Sin embargo, en el producto visible final la navegación histórica y longitudinal del Frontend continúa basada en \texttt{subjectRef}; las pantallas de historial consumen endpoints de \texttt{/api/subjects} y no un flujo browser completo de alta o selección de entidades \texttt{Patient}. Por este motivo, el dominio Patient se clasifica como implementación parcial y no como gestión integral de pacientes end-to-end en interfaz. Cada corrida conserva su linaje técnico; las mediciones y los assets permanecen vinculados a la corrida y al corte de origen; y la revisión profesional se registra sin sobrescribir el resultado automático original, preservando trazabilidad y auditabilidad.
% CROSS_REFERENCE_MAPPING_REQUIRED
...
Desde la perspectiva arquitectónica, el recorrido funcional final es: sujeto o referencia seudónima, Study, DICOM, identificación de series, análisis sagital y axial cuando corresponde, mediciones, evidencia visual, revisión profesional, reporte, historial y comparación longitudinal. El Backend actúa como frontera de producto, seguridad, persistencia y orquestación: valida la solicitud, coordina la ingesta con el AI Module, persiste los contratos resultantes en PostgreSQL, sirve assets autorizados, compone reportes y media el acceso al proveedor LLM. El AI Module permanece como servicio especializado: identifica y clasifica series, registra inputs, ejecuta modelos PyTorch, genera máscaras, overlays y salidas experimentales. La línea P10.7 no debe describirse como inferencia productiva automática: su endpoint existe, pero permanece bloqueado por \texttt{blocked\_preprocessing\_parity} y \texttt{automaticInferenceEnabled=false}. El Frontend presenta posteriormente el estudio en el espacio de revisión profesional, desde donde pueden consultarse resultados persistidos y reabrirse el caso sin repetir necesariamente la inferencia. Los formatos MHA/NIfTI se mantienen como compatibilidad técnica y fixtures de investigación, no como flujo principal de producto.
% CROSS_REFERENCE_MAPPING_REQUIRED
% CROSS_REFERENCE_MAPPING_REQUIRED
...
\hline
\endfoot
Spring Boot / Java 21 & Backend de producto & Se utiliza como frontera de producto para concentrar autenticación, autorización, reglas de negocio, persistencia, auditoría, reportes, chat y contratos públicos en un servicio independiente del runtime científico. Esta separación evita trasladar al Backend las dependencias propias del procesamiento y de los modelos de inteligencia artificial. \\
...
Google Cloud & Arquitectura objetivo y despliegue parcial DEV & Se definió como plataforma objetivo por disponer de servicios administrados para cómputo, persistencia, almacenamiento y gestión operativa. Al cierre técnico sólo están desplegados/validados Artifact Registry/WIF, Backend Cloud Run DEV y Frontend Cloud Run DEV; AI Cloud Run privado, Cloud SQL, Secret Manager, storage productivo, DEMO environment y monitoreo/alerting live permanecen no desplegados. \\
\hline
\end{longtable}
}
...
Durante la integración final se utilizaron dos niveles de infraestructura. El primero fue el stack local containerizado para validar el producto completo con PostgreSQL, AI Module, mock LLM, Backend y Frontend. El segundo fue el despliegue DEV parcial en Google Cloud, donde quedaron corroborados Artifact Registry/WIF y servicios Cloud Run para Backend y Frontend. Esta evidencia no equivale a un despliegue productivo completo: el AI Module privado en Cloud Run, Cloud SQL, Secret Manager, storage productivo, DEMO environment y monitoreo/alerting live permanecen no desplegados.

\subsection{Arquitectura objetivo en Google Cloud}

La plataforma objetivo del sistema es Google Cloud, con una migración organizada por fases para no crear recursos cuyo costo o necesidad no estén justificados. La arquitectura objetivo conecta GitHub Actions con Workload Identity Federation, Artifact Registry, Cloud Run Frontend, Cloud Run Backend y, para una etapa posterior, Cloud Run AI privado protegido con Google ID Token. El Backend se conectaría a Cloud SQL y utilizaría Secret Manager, Cloud Storage y Cloud Monitoring/Logging como servicios gestionados. Al cierre técnico, sólo Artifact Registry/WIF y Cloud Run DEV para Backend y Frontend cuentan con evidencia de despliegue; el resto debe leerse como arquitectura objetivo.
...
El principio financiero adoptado es no crear un recurso cuyo costo máximo no pueda estimarse de antemano. Esta arquitectura debe leerse por estado de madurez: Artifact Registry/WIF, Backend Cloud Run DEV (legacy y Green), Frontend Cloud Run DEV, AI Cloud Run privado, Cloud SQL, Secret Manager y storage durable están desplegados y validados de punta a punta (Capítulo~\ref{chap:resultados-evaluacion}, Sección~\ref{sec:resultados-validacion-final}); DEMO environment y monitoreo/alerting live no están desplegados y no deben presentarse como parte del estado operativo actual. El entorno Cloud se encuentra además pausado intencionalmente al cierre de este documento para minimizar consumo de crédito mientras continúa el trabajo de documentación; el procedimiento de pausa y el de restauración están documentados en el repositorio Backend (\texttt{docs/cloud/CLOUD\_PAUSE\_RESTORE\_COMMANDS.md}).


\subsection{Estado real del despliegue Cloud}

El estado Cloud final se documenta por recurso en la Tabla~\ref{tab:arch-cloud-estado-final}. Esta tabla resume el estado de despliegue de cada recurso; la evidencia técnica completa de extremo a extremo (autenticación privada, persistencia durable, resiliencia ante reinicio, reproceso sin re-subida, costo y performance reales) se desarrolla en el Capítulo~\ref{chap:resultados-evaluacion}, Sección~\ref{sec:resultados-cloud-e2e}.
...
\renewcommand{\arraystretch}{1.18}
\begin{longtable}{>{\raggedright\arraybackslash}p{0.28\textwidth} >{\raggedright\arraybackslash}p{0.20\textwidth} >{\raggedright\arraybackslash}p{0.44\textwidth}}
\caption{Estado final de despliegue Cloud por recurso.}\label{tab:arch-cloud-estado-final}\\
\hline
\textbf{Recurso} & \textbf{Estado} & \textbf{Lectura para la tesis} \\
\hline
\endfirsthead
\hline
\textbf{Recurso} & \textbf{Estado} & \textbf{Lectura para la tesis} \\
...
\hline
\endfoot
Artifact Registry y Workload Identity Federation & Desplegado / validado & Existe evidencia de publicacion de imagenes y federacion CI hacia Google Cloud. \\
Backend Cloud Run DEV (legacy y Green) & Desplegado / validado & Dos servicios: legacy (\texttt{pfi-backend-dev}) y Green (\texttt{pfi-backend-v2-dev}); el Frontend fue conmutado a Green (Capítulo~\ref{chap:resultados-evaluacion}). No implica ambiente productivo ni DEMO final. \\
Frontend Cloud Run DEV & Desplegado / validado & Servicio DEV corroborado, apuntando en runtime a Backend Green; no implica ambiente productivo ni DEMO final. \\
AI Cloud Run privado & Desplegado / validado & Autenticacion con Google ID Token ejercitada de punta a punta contra el servicio AI privado real; acceso no autenticado verificado en 403. \\
Cloud SQL & Desplegado / validado & Utilizada como persistencia gestionada real (Postgres) para el Backend Green, con durabilidad verificada ante reinicio. \\
Secret Manager & Desplegado / validado & \texttt{DATABASE\_URL} y el secreto JWT del Backend Green se sirven desde Secret Manager, no desde variables en texto plano. \\
Storage durable (GCS) & Desplegado / validado & Dos buckets en uso real: assets durables del producto y checkpoint del modelo, con integridad verificada por tamaño tras materializacion. \\
DEMO environment & No desplegado & No existe promocion DEV--DEMO ejecutada. \\
Monitoring/alerting live & No desplegado & La observabilidad esta disenada, pero no operada con trafico real ni alertas live. \\
\hline
\end{longtable}
}
