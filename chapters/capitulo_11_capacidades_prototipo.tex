...
\section{Demo Mode y validacion funcional reproducible}
\label{sec:cap-demo-mode}

Demo Mode es un recorrido deterministico, no clinico, construido para defensa academica, testing E2E y validacion funcional del producto. Utiliza datos sinteticos o fixtures controlados y no debe presentarse como sustituto de inferencia clinica real. La configuracion final contiene cuatro studies y dos subjects de demostracion; cuando el localizador real no puede habilitarse, el flujo usa \texttt{localizationStatus=demo\_fixture}, \texttt{source=DEMO\_FIXTURE}, \texttt{validated=false} y una advertencia explicita de no inferencia real.

El valor de Demo Mode es que permite recorrer de punta a punta el producto sin fabricar un nivel anatomico no validado. En ese recorrido se ejercitan ingesta, corrida AI, mediciones, evidencia visual, revision profesional, reporte, exportacion PDF/JSON, chat contextual contra proveedor mock y comparacion longitudinal deterministica. La comparacion longitudinal usa valores controlados por \texttt{caseId}; por ejemplo, el flujo final valida una variacion absoluta de 0,6 mm luego de una correccion profesional. El resultado browser E2E final fue Playwright 11/11 PASS, interpretado como validacion funcional del prototipo y no como validacion clinica.

\section{Disponibilidad productiva de modelos y extensiones}

La Tabla~\ref{tab:cap-modelos-disponibilidad-final} resume la diferencia entre evaluacion offline, disponibilidad runtime y exposicion de producto. Esta distincion evita presentar una metrica experimental como si habilitara automaticamente una capacidad clinica o productiva.
...
\hline
\textbf{Capacidad} & \textbf{Evidencia} & \textbf{Estado} & \textbf{Limitacion principal} \\
\hline
\endfirsthead
\hline
\textbf{Capacidad} & \textbf{Evidencia} & \textbf{Estado} & \textbf{Limitacion principal} \\
...
\hline
\endfoot
Sagital SPIDER U-Net2D & Test Dice macro 0,8934 e IoU 0,8080; quality gate aprobado & Validada tecnicamente & Dataset academico, sin validacion clinica. \\
...
67B2 localizacion lumbar absoluta & DEV top1 0,9343629343629344; top2 0,972972972972973; MRR 0,9617117117117117; n=259. Locked Top1 0,9049295774647887; Top2 0,9612676056338029; MRR 0,9448356807511736; pipeline accuracy given emit 0,9909502262443439 & Parcial/offline & \texttt{67B2\_RUNTIME\_PARITY\_NOT\_PROVEN}; \texttt{productEnabled=false}; \texttt{parityVerified=false}. \\
67C pairing sagital--axial & Algoritmo geometrico deterministico con flags de contrato & Experimental & \texttt{experimental=true}, \texttt{validated=false}, \texttt{reviewRequired=true}; depende de 67B2. \\
...
Grad-CAM & Tests AI especificos PASS, contrato API validado, wiring Frontend presente & Experimental & No fue ejercitado explicitamente en el browser E2E final; no es validacion clinica. \\
\hline
\end{longtable}
}

\section{Capacidades extendidas de revision y analisis}

Ademas del flujo principal de segmentacion, medicion y reporte, el producto final incorpora extensiones de revision con alcance controlado. Concordance compara evidencia del mismo fenomeno, nivel y lado como regla descriptiva, no clinica. Review analytics agrega telemetria de revision para observar tiempos y estados, sin medir exactitud medica. Feedback curation registra candidatos de curacion profesional, pero no activa reentrenamiento automatico ni aprendizaje online. Grad-CAM expone mapas experimentales para el clasificador subarticular, siempre con \texttt{experimental=true}. Product-analysis integra resultados del flujo de producto: en Demo Mode puede completar un nivel deterministico, mientras que en entradas reales conserva el fallo seguro cuando 67B2 no prueba paridad runtime.

El chat contextual usa una abstraccion de proveedor LLM. En el recorrido demo se valida contra un mock deterministico; Qwen/self-hosted se documenta como target soportado, no como despliegue productivo ejercitado. Esta capacidad acompana la lectura del caso, pero no genera diagnostico, tratamiento ni decision automatica.
...
Persistencia, reapertura y trazabilidad & 11.9 & Implementadas; Patient/API/persistencia validado en Backend, con UI longitudinal todavia basada principalmente en \texttt{subjectRef} \\
...
Recorrido extremo a extremo con interfaz & --- & Validado funcionalmente en Playwright 11/11 sobre Demo Mode y recorrido de producto; no equivale a validacion clinica \\
Validación profesional sobre el producto & — & Pendiente \\
Comparación longitudinal & — & Implementada y validada en flujo demo/\texttt{subjectRef}; los inputs reales no-demo dependen de localizacion anatomica resuelta \\
Despliegue en la plataforma objetivo & — & Parcial: Artifact Registry/WIF y Cloud Run DEV Backend/Frontend desplegados; AI privado, Cloud SQL, Secret Manager, storage productivo, DEMO environment y monitoreo live no desplegados \\
\hline
\end{longtable}
}
