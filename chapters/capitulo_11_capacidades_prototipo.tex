...
Cómo funciona. Antes de iniciar un nuevo análisis, el profesional define el \texttt{caseId} y una referencia seudónima de sujeto para el Study. En Backend existe además el dominio \texttt{Patient}: entidad persistida, endpoint \texttt{/api/patients}, campo \texttt{Study.patientId} y asociación Patient--Study validada a nivel API. Sin embargo, la interfaz final de historial y longitudinal continúa operando principalmente con \texttt{subjectRef}; no existe todavía un flujo browser completo de alta o selección de entidades \texttt{Patient} persistidas. Por eso la capacidad se describe como implementación parcial del dominio Patient. La vía principal de ingesta continúa siendo un estudio en formato DICOM empaquetado, del que el sistema extrae las series y determina automáticamente cuáles son compatibles con los modelos disponibles. El usuario no selecciona imágenes de a una: la cantidad de cortes se obtiene del propio volumen. Se conserva compatibilidad con otros formatos, empleados como fixtures de prueba y en flujos de investigación.
...
% DOCX_FIGURE_NUMBER: 11.1
% DOCX_CAPTION: Acceso con estado de cuenta pendiente de aprobación y worklist de estudios. Fuente: elaboración propia.
% WORD_PARAGRAPH: 911
% DRAWING_COUNT: 0
...
\addcontentsline{lof}{figure}{\protect\numberline{11.1}Acceso con estado de cuenta pendiente de aprobación y worklist de estudios. Fuente: elaboración propia.}
% FIGURE_PENDING_MIGRATION
% DOCX_FIGURE_NUMBER: 11.2
% DOCX_CAPTION: Carga de un estudio DICOM con sus series identificadas y clasificadas. Fuente: elaboración propia.
% WORD_PARAGRAPH: 913
% DRAWING_COUNT: 0
% RELATIONSHIP_IDS: NONE
% MEDIA_FILES: NONE
...
% SOURCE_VISUAL_MISSING_FROM_DOCX
% VISUAL_SOURCE_REQUIRED_FROM_AUTHOR: YES
\addcontentsline{lof}{figure}{\protect\numberline{11.2}Carga de un estudio DICOM con sus series identificadas y clasificadas. Fuente: elaboración propia.}
...
% FIGURE_PENDING_MIGRATION
% DOCX_FIGURE_NUMBER: 11.3
% DOCX_CAPTION: Workspace sagital con la máscara superpuesta sobre la imagen original. Fuente: elaboración propia.
% WORD_PARAGRAPH: 923
% DRAWING_COUNT: 0
% RELATIONSHIP_IDS: NONE
% MEDIA_FILES: NONE
...
% VISUAL_SOURCE_REQUIRED_FROM_AUTHOR: YES
% CROSS_REFERENCE_MAPPING_REQUIRED
\addcontentsline{lof}{figure}{\protect\numberline{11.3}Workspace sagital con la máscara superpuesta sobre la imagen original. Fuente: elaboración propia.}
...
% FIGURE_PENDING_MIGRATION
% DOCX_FIGURE_NUMBER: 11.4
% DOCX_CAPTION: Workspace axial con la máscara superpuesta sobre la imagen original. Fuente: elaboración propia.
% WORD_PARAGRAPH: 936
% DRAWING_COUNT: 0
% RELATIONSHIP_IDS: NONE
% MEDIA_FILES: NONE
...
% SOURCE_VISUAL_MISSING_FROM_DOCX
% VISUAL_SOURCE_REQUIRED_FROM_AUTHOR: YES
\addcontentsline{lof}{figure}{\protect\numberline{11.4}Workspace axial con la máscara superpuesta sobre la imagen original. Fuente: elaboración propia.}
Qué evidencia lo respalda. El gate oficial del artifact axial registra \texttt{qualityGatePassed=false}. La métrica oficial de primer plano es Dice macro \(0{,}679348283374592\) e IoU macro \(0{,}5699727598420694\), con razón \texttt{dice\_macro\_foreground\_below\_threshold}. Como análisis secundario, excluyendo la clase \texttt{raw\_0}, el Dice es \(0{,}8105661875685084\) y el IoU \(0{,}6915400877715925\). Esta segunda lectura ayuda a interpretar el comportamiento sobre estructuras anatómicas, pero no reemplaza la métrica oficial del gate.
...
% FIGURE_PENDING_MIGRATION
% DOCX_FIGURE_NUMBER: 11.5
% DOCX_CAPTION: Panel de mediciones con la estructura, el plano y el corte de origen de cada valor. Fuente: elaboración propia.
% WORD_PARAGRAPH: 952
% DRAWING_COUNT: 0
% RELATIONSHIP_IDS: NONE
% MEDIA_FILES: NONE
% FIGURE_TYPE: MEASUREMENT_VIEW
% SOURCE_VISUAL_MISSING_FROM_DOCX
% VISUAL_SOURCE_REQUIRED_FROM_AUTHOR: YES
% CROSS_REFERENCE_MAPPING_REQUIRED
\addcontentsline{lof}{figure}{\protect\numberline{11.5}Panel de mediciones con la estructura, el plano y el corte de origen de cada valor. Fuente: elaboración propia.}
...
% FIGURE_PENDING_MIGRATION
% DOCX_FIGURE_NUMBER: 11.6
% DOCX_CAPTION: Navegación por cortes del volumen con el resultado vinculado a su corte. Fuente: elaboración propia.
% WORD_PARAGRAPH: 973
% DRAWING_COUNT: 0
% RELATIONSHIP_IDS: NONE
% MEDIA_FILES: NONE
% FIGURE_TYPE: RESULT_VIEW
% SOURCE_VISUAL_MISSING_FROM_DOCX
% VISUAL_SOURCE_REQUIRED_FROM_AUTHOR: YES
\addcontentsline{lof}{figure}{\protect\numberline{11.6}Navegación por cortes del volumen con el resultado vinculado a su corte. Fuente: elaboración propia.}
Qué evidencia lo respalda. El procesamiento de la serie completa está implementado y los assets se persisten y se sirven por corte. El frontend consume la integración contractual P10.9 y presenta el catálogo full-series en un visor navegable. Las pruebas verifican la cantidad y el orden de los cortes, el eje, el índice computacional y la numeración mostrada, con índice interno desde cero y numeración de interfaz desde uno.

Qué límites tiene. La superposición automática no se presupone para todos los cortes de todas las series: solo se muestra donde hay resultado, y los cortes sin inferencia lo declaran en lugar de presentarse vacíos. El visor no simula una correspondencia espacial entre el plano sagital y el axial cuando la metadata geométrica no permite demostrarla; la relación entre planos se establece por la asignación de nivel de la sección~\ref{sec:cap-asignacion-nivel-anatomico} y no por una superposición supuesta. La integración de los contratos congelados en el frontend está implementada. El recorrido browser E2E final obtuvo 11/11 en Playwright sobre el stack final, pero ese resultado valida el flujo funcional del prototipo y Demo Mode; no convierte a Patient en una entidad browser-level completa ni habilita inferencia clínica.
...
% FIGURE_PENDING_MIGRATION
% DOCX_FIGURE_NUMBER: 11.7
% DOCX_CAPTION: Controles de revisión profesional sobre una estructura segmentada. Fuente: elaboración propia.
% WORD_PARAGRAPH: 983
% DRAWING_COUNT: 0
% RELATIONSHIP_IDS: NONE
% MEDIA_FILES: NONE
...
% SOURCE_VISUAL_MISSING_FROM_DOCX
% VISUAL_SOURCE_REQUIRED_FROM_AUTHOR: YES
\addcontentsline{lof}{figure}{\protect\numberline{11.7}Controles de revisión profesional sobre una estructura segmentada. Fuente: elaboración propia.}
% FIGURE_PENDING_MIGRATION
% DOCX_FIGURE_NUMBER: 11.8
% DOCX_CAPTION: Preinforme automático revisable con estructuras, mediciones y estado de revisión. Fuente: elaboración propia.
% WORD_PARAGRAPH: 985
% DRAWING_COUNT: 0
% RELATIONSHIP_IDS: NONE
% MEDIA_FILES: NONE
...
% VISUAL_SOURCE_REQUIRED_FROM_AUTHOR: YES
% CROSS_REFERENCE_MAPPING_REQUIRED
\addcontentsline{lof}{figure}{\protect\numberline{11.8}Preinforme automático revisable con estructuras, mediciones y estado de revisión. Fuente: elaboración propia.}
...
Qué límites tiene. El preinforme es un punto de partida editable y no un informe clínico: su función es reducir la transcripción manual de valores y organizarlos en un orden anatómico previsible, mientras la redacción final, la interpretación y la responsabilidad permanecen en el profesional. La utilidad del flujo de revisión sobre el producto integrado se observó en las dos sesiones de revisión cualitativa documentadas en la sección~\ref{sec:resultados-validacion-profesional-cualitativa}; esas sesiones no equivalen a validación clínica, y la validación cruzada final, con lectura profesional independiente sobre los mismos estudios, continúa pendiente.
...
Cómo funciona. Un estudio actúa como contenedor lógico y una corrida representa una ejecución concreta, de manera que ejecutar de nuevo no sobrescribe la evidencia anterior. Backend permite vincular Studies con una entidad Patient mediante \texttt{patientId}, y esa asociación se persiste y se valida por API. En la interfaz final, sin embargo, el historial y la comparación longitudinal siguen recorriendo principalmente la identidad \texttt{subjectRef}. Por lo tanto, \texttt{patientId} debe leerse como dominio de Backend implementado, no como flujo visual completo de gestión de pacientes. Cada corrida genera identificadores que permiten seguirla: el del caso, el de la ejecución, el que agrupa ambos planos y el que correlaciona solicitudes y registros técnicos.

% FIGURE_PENDING_MIGRATION
% DOCX_FIGURE_NUMBER: 11.9
% DOCX_CAPTION: Historial de corridas y reapertura de un estudio ya procesado. Fuente: elaboración propia.
% WORD_PARAGRAPH: 994
% DRAWING_COUNT: 0
% RELATIONSHIP_IDS: NONE
% MEDIA_FILES: NONE
...
% SOURCE_VISUAL_MISSING_FROM_DOCX
% VISUAL_SOURCE_REQUIRED_FROM_AUTHOR: YES
\addcontentsline{lof}{figure}{\protect\numberline{11.9}Historial de corridas y reapertura de un estudio ya procesado. Fuente: elaboración propia.}
...
Qué límites tiene. Las corridas anteriores a la incorporación del catálogo de cortes conservan únicamente los assets que existían en su momento y se abren en modo de compatibilidad, sin fabricar un catálogo que no se generó. Los registros históricos pueden no tener un Patient asociado, porque \texttt{patientId} es nullable y no se infiere retroactivamente desde \texttt{subjectRef}. Cuando la evidencia ya existe, la reapertura utiliza los datos persistidos y no requiere repetir la inferencia.
...
% FIGURE_PENDING_MIGRATION
% DOCX_FIGURE_NUMBER: 11.10
% DOCX_CAPTION: Estado degradado informado de forma explícita ante indisponibilidad del módulo de inferencia. Fuente: elaboración propia.
% WORD_PARAGRAPH: 1004
% DRAWING_COUNT: 0
% RELATIONSHIP_IDS: NONE
% MEDIA_FILES: NONE
% FIGURE_TYPE: UI_SCREENSHOT
\addcontentsline{lof}{figure}{\protect\numberline{11.10}Estado degradado informado de forma explícita ante indisponibilidad del módulo de inferencia. Fuente: elaboración propia.}
% SOURCE_VISUAL_MISSING_FROM_DOCX
% VISUAL_SOURCE_REQUIRED_FROM_AUTHOR: YES
...
\section{Demo Mode y validación funcional reproducible}
\label{sec:cap-demo-mode}

Demo Mode es un recorrido determinístico, no clínico, construido para defensa académica, testing E2E y validación funcional del producto. Utiliza datos sintéticos o fixtures controlados y no debe presentarse como sustituto de inferencia clínica real. La configuración final contiene cuatro studies y dos subjects de demostración; cuando el localizador real no puede habilitarse, el flujo usa \texttt{localizationStatus=demo\_fixture}, \texttt{source=DEMO\_FIXTURE}, \texttt{validated=false} y una advertencia explícita de no inferencia real.

El valor de Demo Mode es que permite recorrer de punta a punta el producto sin fabricar un nivel anatómico no validado. En ese recorrido se ejercitan ingesta, corrida AI, mediciones, evidencia visual, revisión profesional, reporte, exportación PDF/JSON, chat contextual contra proveedor mock y comparación longitudinal determinística. La comparación longitudinal usa valores controlados por \texttt{caseId}; por ejemplo, el flujo final valida una variación absoluta de 0,6 mm luego de una corrección profesional. El resultado browser E2E final fue Playwright 11/11 PASS, interpretado como validación funcional del prototipo y no como validación clínica.

\section{Disponibilidad productiva de modelos y extensiones}

La Tabla~\ref{tab:cap-modelos-disponibilidad-final} resume la diferencia entre evaluación offline, disponibilidad runtime y exposición de producto. Esta distinción evita presentar una métrica experimental como si habilitara automáticamente una capacidad clínica o productiva.
...
\setlength{\tabcolsep}{3pt}
\renewcommand{\arraystretch}{1.18}
\begin{longtable}{>{\raggedright\arraybackslash}p{0.18\textwidth} >{\raggedright\arraybackslash}p{0.22\textwidth} >{\raggedright\arraybackslash}p{0.22\textwidth} >{\raggedright\arraybackslash}p{0.30\textwidth}}
\caption{Estado final de modelos y extensiones.}\label{tab:cap-modelos-disponibilidad-final}\\
\hline
\textbf{Capacidad} & \textbf{Evidencia} & \textbf{Estado} & \textbf{Limitación principal} \\
\hline
\endfirsthead
\hline
\textbf{Capacidad} & \textbf{Evidencia} & \textbf{Estado} & \textbf{Limitación principal} \\
...
\hline
\endfoot
Sagital SPIDER U-Net2D & Test Dice macro 0,8934 e IoU 0,8080; quality gate aprobado & Validada técnicamente & Dataset académico, sin validación clínica. \\
Axial Sudirman/Al-Kafri U-Net2D & Dice oficial 0,679348283374592; IoU 0,5699727598420694 & Experimental & \texttt{qualityGatePassed=false}; la lectura excluyendo \texttt{raw\_0} no reemplaza el gate. \\
67B2 localización lumbar absoluta & DEV top1 0,9343629343629344; top2 0,972972972972973; MRR 0,9617117117117117; n=259. Locked Top1 0,9049295774647887; Top2 0,9612676056338029; MRR 0,9448356807511736; pipeline accuracy given emit 0,9909502262443439 & Parcial/offline & \texttt{67B2\_RUNTIME\_PARITY\_NOT\_PROVEN}; \texttt{productEnabled=false}; \texttt{parityVerified=false}. \\
67C pairing sagital--axial & Algoritmo geométrico determinístico con flags de contrato & Experimental & \texttt{experimental=true}, \texttt{validated=false}, \texttt{reviewRequired=true}; depende de 67B2. \\
P10.7 hallazgos degenerativos & Checkpoints y contratos existen & Bloqueada & \texttt{blocked\_preprocessing\_parity}; \texttt{automaticInferenceEnabled=false}. \\
Grad-CAM & Tests AI específicos PASS, contrato API validado, wiring Frontend presente & Experimental & No fue ejercitado explícitamente en el browser E2E final; no es validación clínica. \\
\hline
\end{longtable}
}

\section{Capacidades extendidas de revisión y análisis}

Además del flujo principal de segmentación, medición y reporte, el producto final incorpora extensiones de revisión con alcance controlado. Concordance compara evidencia del mismo fenómeno, nivel y lado como regla descriptiva, no clínica. Review analytics agrega telemetría de revisión para observar tiempos y estados, sin medir exactitud médica. Feedback curation registra candidatos de curación profesional, pero no activa reentrenamiento automático ni aprendizaje online. Grad-CAM expone mapas experimentales para el clasificador subarticular, siempre con \texttt{experimental=true}. Product-analysis integra resultados del flujo de producto: en Demo Mode puede completar un nivel determinístico, mientras que en entradas reales conserva el fallo seguro cuando 67B2 no prueba paridad runtime.

El chat contextual usa una abstracción de proveedor LLM. En el recorrido demo se valida contra un mock determinístico; Qwen/self-hosted se documenta como target soportado, no como despliegue productivo ejercitado. Esta capacidad acompaña la lectura del caso, pero no genera diagnóstico, tratamiento ni decisión automática.
...
Visualización y navegación por cortes & 11.7 & Implementada; P10.9 integrado en frontend con catálogo full-series y visor navegable; Playwright final 11/11 PASS en recorrido funcional del prototipo \\
...
Persistencia, reapertura y trazabilidad & 11.9 & Implementadas; Patient/API/persistencia validado en Backend, con UI longitudinal todavía basada principalmente en \texttt{subjectRef} \\
...
Recorrido extremo a extremo local/containerizado & --- & Validado según la evidencia de la sección~\ref{sec:resultados-validacion-final} (Playwright 11/11 sobre Demo Mode y recorrido de producto); no equivale a validación clínica \\
Recorrido extremo a extremo en Cloud desde navegador & --- & Parcial, no cerrado, según la evidencia de la sección~\ref{sec:resultados-cloud-e2e} \\
Validación profesional sobre el producto & — & Revisión cualitativa realizada (Dra. Susana Torres y Dra. Claudia Cejas, sección~\ref{sec:resultados-validacion-profesional-cualitativa}); no es validación clínica; la validación cruzada final continúa pendiente \\
Comparación longitudinal & — & Parcial: disponible en el alcance Demo/\texttt{subjectRef} según la evidencia existente; no constituye validación clínica de progresión \\
Despliegue en Google Cloud & — & Entorno DEV desplegado y validado (Backend, Frontend, AI privado, Cloud SQL, Secret Manager y storage durable); DEMO environment separado y monitoreo/alerting en vivo no desplegados \\
\hline
\end{longtable}
}
