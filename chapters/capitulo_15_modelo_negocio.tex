...
La evidencia técnica medida de la primera categoría proviene, concretamente, de los estudios sintéticos utilizados en el Capítulo~\ref{chap:resultados-evaluacion} para medir el costo real de Google Cloud; esa aclaración no constituye una categoría adicional, sino la fuente de la primera.
...
\begin{longtable}{>{\raggedright\arraybackslash}p{0.18\textwidth} >{\raggedright\arraybackslash}p{0.33\textwidth} >{\raggedright\arraybackslash}p{0.38\textwidth}}
...
\label{sec:mn-branding}

El producto se presenta comercialmente bajo el nombre MIRA·L, que sintetiza \textit{Mapeo Integrado y Revisión Anatómica Lumbar}. La construcción del nombre admite además una segunda lectura, no excluyente de la primera: \enquote{MIRA} remite a observar, revisar y analizar imágenes, mientras que la \enquote{L} delimita explícitamente el dominio lumbar del prototipo. El nombre no se presenta como marca registrada; se trata de una identidad comercial propuesta para este trabajo.

El isotipo está compuesto por cinco módulos geométricos horizontales organizados verticalmente. Los cinco módulos constituyen una referencia abstracta a los cinco niveles lumbares, sin intentar reproducir anatomía real: no representan vértebras, no representan una lesión específica y no representan un resultado diagnóstico. La organización modular remite además a un análisis estructurado por niveles, a un procesamiento reproducible y a la integración de información anatómica en un mismo sistema visual. Esta abstracción fue elegida deliberadamente para evitar asociaciones con diagnóstico autónomo, con lesión específica, con rehabilitación o kinesiología, y con la iconografía médica genérica (cruces, estetoscopios, siluetas de columna). El isotipo se lee primero como sistema modular técnico y, en una segunda lectura, como referencia al dominio lumbar del producto.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.7\textwidth]{images/branding/miral_logo_principal.png}
\caption{Identidad visual principal de MIRA·L --- Mapeo Integrado y Revisión Anatómica Lumbar. El isotipo modular refiere de manera abstracta a la organización por niveles lumbares, mientras que el wordmark conserva el protagonismo de la marca. Fuente: elaboración propia.}
\label{fig:branding-miral-logo}
\end{figure}

El wordmark utiliza una construcción tipográfica sans serif sobria y de alta legibilidad, en la que el nombre tiene mayor protagonismo visual que el isotipo. El punto medio que separa \enquote{MIRA} de \enquote{L} forma parte de la identidad y articula tipográficamente el nombre; no debe interpretarse como un símbolo de detección, de un objetivo (\textit{target}) ni de un resultado o hallazgo.

La paleta cromática se resume en la Tabla~\ref{tab:mn-branding-paleta}. Su función se justifica por criterios de contraste, consistencia y legibilidad, y no por atributos psicológicos no verificables del color.
...
\setlength{\tabcolsep}{3pt}
\renewcommand{\arraystretch}{1.18}
\begin{longtable}{>{\raggedright\arraybackslash}p{0.16\textwidth} >{\raggedright\arraybackslash}p{0.14\textwidth} >{\raggedright\arraybackslash}p{0.60\textwidth}}
\caption{Paleta cromática de MIRA·L y función de cada color en el sistema.}\label{tab:mn-branding-paleta}\\
\hline
\textbf{Color} & \textbf{Valor} & \textbf{Función en el sistema} \\
\hline
\endfirsthead
\hline
\textbf{Color} & \textbf{Valor} & \textbf{Función en el sistema} \\
...
\hline
\endfoot
Grafito & \texttt{\#282E34} & Color del wordmark y de la jerarquía tipográfica institucional; aporta el contraste principal sobre fondo claro. \\
Azul petróleo & \texttt{\#176B78} & Color de acento e identificación del isotipo; distingue el símbolo del texto sin competir con él. \\
Blanco & \texttt{\#FFFFFF} & Fondo principal; garantiza claridad y contraste máximo con grafito y azul petróleo. \\
Gris auxiliar & \texttt{\#EEF1F3} & Superficies secundarias, fondos de documentación y áreas de baja jerarquía visual. \\
\hline
\end{longtable}
}

La identidad acompaña el posicionamiento del producto como software técnico para revisión anatómica asistida, y no como una herramienta de diagnóstico autónomo: el profesional revisa, puede corregir y conserva la interpretación final, según el esquema \textit{human-in-the-loop} descrito en este capítulo. Esta condición es consistente con el modelo B2B institucional presentado en la Tabla~\ref{tab:mn-roles}: el usuario que opera el sistema es el profesional de diagnóstico por imágenes, mientras que quien contrata y paga el servicio es la institución. La identidad visual está pensada para funcionar de manera consistente en la interfaz web del producto, en documentación técnica, en presentaciones técnico-comerciales dirigidas a instituciones, en el favicon del sistema y en material institucional impreso o digital. Un mensaje complementario de marca, sin carácter obligatorio dentro del logotipo, sintetiza este posicionamiento: \enquote{Análisis asistido para revisión profesional}.

El sistema de identidad contempla, además del logo principal, un conjunto de variantes de uso: una versión horizontal reducida, una versión compacta, el isotipo aislado, versiones de favicon en 16, 32 y 48 píxeles, una versión monocromática y una versión en blanco sobre fondo oscuro para su uso en superficies de color oscuro. Estas variantes garantizan la consistencia de la marca en distintos formatos y tamaños sin necesidad de reproducir aquí cada una como figura independiente.
...
\setlength{\tabcolsep}{3pt}
\renewcommand{\arraystretch}{1.18}
\begin{longtable}{>{\raggedright\arraybackslash}p{0.13\textwidth} >{\raggedright\arraybackslash}p{0.11\textwidth} >{\raggedright\arraybackslash}p{0.11\textwidth} >{\raggedright\arraybackslash}p{0.11\textwidth} >{\raggedright\arraybackslash}p{0.20\textwidth} >{\raggedright\arraybackslash}p{0.20\textwidth}}
\caption{Planes comerciales propuestos (Base Comercial Objetivo).}\label{tab:mn-planes}\\
\hline
...
El componente Cloud del costo variable no es una estimación teórica: proviene de una medición real ejecutada sobre el entorno de Google Cloud desplegado para este proyecto (Backend, IA privada, Cloud SQL y almacenamiento durable en GCS), utilizando una \textit{fixture} DICOM sintética y no identificable, en 35 ejecuciones exitosas distribuidas en dos lotes independientes de 10 y 25 estudios, cuyos resultados convergieron de forma consistente. La validación técnica de extremo a extremo que sustenta esta medición (autenticación privada, persistencia durable, reinicio y reproceso) se presenta en el Capítulo~\ref{chap:resultados-evaluacion}, Sección~\ref{sec:resultados-cloud-e2e}; esta sección reutiliza esa evidencia para su traducción económica, sin remedirla.
...
\caption{Costos Cloud observados (evidencia experimental, Etapa 10/10B).}\label{tab:mn-costos-cloud}\\
\hline
...
\setlength{\tabcolsep}{3pt}
\renewcommand{\arraystretch}{1.18}
\begin{longtable}{>{\raggedright\arraybackslash}p{0.27\textwidth} >{\raggedright\arraybackslash}p{0.27\textwidth} >{\raggedright\arraybackslash}p{0.37\textwidth}}
...
\setlength{\tabcolsep}{3pt}
\renewcommand{\arraystretch}{1.18}
\begin{longtable}{>{\raggedright\arraybackslash}p{0.19\textwidth} >{\raggedright\arraybackslash}p{0.13\textwidth} >{\raggedright\arraybackslash}p{0.13\textwidth} >{\raggedright\arraybackslash}p{0.13\textwidth} >{\raggedright\arraybackslash}p{0.12\textwidth} >{\raggedright\arraybackslash}p{0.12\textwidth}}
...
\setlength{\tabcolsep}{3pt}
\renewcommand{\arraystretch}{1.18}
\begin{longtable}{>{\raggedright\arraybackslash}p{0.27\textwidth} >{\raggedright\arraybackslash}p{0.21\textwidth} >{\raggedright\arraybackslash}p{0.21\textwidth} >{\raggedright\arraybackslash}p{0.21\textwidth}}
\caption{Sensibilidad del VAN sobre el escenario Base.}\label{tab:mn-sensibilidad}\\
\hline
...
\caption{Base conservador frente a Base Comercial Objetivo.}\label{tab:mn-base-vs-objetivo}\\
\hline
